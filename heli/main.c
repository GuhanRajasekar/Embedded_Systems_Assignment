...
main(void)
{
    ...
    while(1)
    {
        ...
        if(g_ui32PrintSkipCounter >= PRINT_SKIP_COUNT)
        {
            ...
            //UARTprintf("\n %20d.%020d ",pfData);
            sprintf(roll, "%3d.%03d", i32IPart[9], i32FPart[9]);
            UARTprintf("\n Roll: %s\n", roll);
            sprintf(pitch, "%3d.%03d", i32IPart[10], i32FPart[10]);
            UARTprintf("\n Pitch: %s\n", pitch);
            sprintf(yaw, "%3d.%03d", i32IPart[11], i32FPart[11]);
            UARTprintf("\n Yaw: %s\n", yaw);
        }
    }
}
