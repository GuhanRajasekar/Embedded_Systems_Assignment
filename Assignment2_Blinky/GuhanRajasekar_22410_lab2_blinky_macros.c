...
void read_sw1()
{
    ...
    if((current_state_sw1) == 0x00)                        // sw1 is pressed but not released
    {
        while(current_state_sw1 == 0x00)
        {
            current_state_sw1 = GPIO_PORTF_DATA_R & 0x10;  // read the data from sw1 again
            if(current_state_sw1 == 0x10)                  // If value is 0x10 => sw1 was released => sw1 was pressed and released => Consider as a valid sw1 press
            {
...
void read_sw2()
{
    ...
    sw2_pressed = 0;
    if(current_state_sw2 == 0x00)      // sw2 is pressed but not released
    {
        while(current_state_sw2 == 0x00)
        {
            current_state_sw2 = GPIO_PORTF_DATA_R & 0x01;  // read the data from sw2 again
            if(current_state_sw2 == 0x01)                  // If value is 0x01 => sw2 has been released => sw2 has been pressed and released => Consider this as a valid sw2 press
            {
